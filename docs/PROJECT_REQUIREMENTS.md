...
- The original delivery-ledger increment excluded refunds, real transfers, external providers, uploads, applicant acknowledgements/disputes, cancellation shortcuts, completion, and public impact publishing. Subsequent approved increments implement private completion and optional immutable public impact publication as specified below; remaining exclusions and existing expiry behavior are unchanged.
...
### Approved private reporting increment

- Implement one screen-only administrative report at `GET /admin/reports`. CSV and PDF exports are deferred until after the final review/design phase, but must be delivered before final project delivery. No chart library or broad administration redesign belongs to this increment.
- Active `admin` and `super_admin` accounts without a required password change may use the separate `viewReports` ability regardless of reviewer assignment. This grants global delivery aggregates only; individual coordination and delivery permissions remain unchanged.
- Use UTC calendar dates. Default to today and the preceding 29 days. Require both custom dates, reject invalid/reversed/future dates, and limit the inclusive range to 366 days. Use half-open boundaries from start midnight through midnight after the end date. Group only by day or month, with zero-filled buckets. Today's figures are partial.
- Strictly allow only `start_date`, `end_date`, `group_by`, `campaign_page`, and `category_page`. Fix both table page sizes at 25, bound page numbers to 1–10,000, and preserve only validated parameters in links. Reject extra parameters and request bodies without flashing input.
- Display lifetime succeeded donation amount/count; selected-period succeeded amount/count and daily/monthly buckets using `paid_at`, never creation time; and period funding by currently active nondeleted category, including zero-total categories and a separate inactive/deleted-category subtotal. Categories describe current assignment, not donation-time history.
- Preserve historical succeeded donations irrespective of guest/anonymity, disabled related accounts, campaign expiry/status/deletion, or category visibility. Donation status is the contribution authority; payment attempts are never counted as contributions. Refund reporting remains deferred.
- Display current counts for all five Donation statuses, all seven Campaign statuses (`draft`, `active`, `paused`, `funded`, `aid_delivery`, `completed`, `cancelled`), and all twelve Help Application statuses. These are current distributions, not historical status reconstructions. Exclude soft-deleted campaigns from operational counts and display their archived count separately. Unknown raw statuses appear only as an unrecognized-status count.
- Reuse Campaign status counts as the current outcome summary; do not invent success/failure rates. Count active nondeleted campaigns with `expires_at` at or before generation as “Active campaigns past expiry,” a subset of active campaigns. Expiry never reclassifies a completed campaign.
- Show paginated lifetime funding reconciliation for every Campaign, including archived records: successful donation ledger amount, stored raised amount, target amount, and signed stored-minus-ledger difference. Never trust the stored total without reconciliation or repair records during reporting.
- Show global lifetime validated terminal simulated-delivery amount/count and undelivered raised balance (lifetime succeeded funding minus validated delivered amount). This is not a bank balance and includes campaigns not yet ready for delivery. Do not substitute fundraising target shortfall for this balance. Show coherent linked completed Campaign/Application counts independently of optional public impact publication.
- Check delivery linkage, terminal state, canonical SDG money, ordered transition revisions/timestamps, sequential instalments, matching proof existence/time, parent state, and funding. Proof contents, references, methods, notes, and private details never reach the report. Completed assistance additionally requires consistent parent completion times, released open slot, and exact ledger/target/stored/delivered agreement. Identify first/subsequent instalments by observed delivery ID ordering per campaign (uniquely linked to coordination), including rejected entries and across chunks, rather than by validated delivery count. An invalid earlier entry withholds the aggregate but must not manufacture a first-entry warning for a later valid entry.
- Use live queries in one consistent read transaction without row locks or summary tables. On operational MySQL/MariaDB use per-transaction REPEATABLE READ; reject nesting that cannot establish that snapshot. Never change or restore the connection session timezone: preserve the established timestamp read/write convention for mixed TIMESTAMP lifecycle fields and DATETIME delivery fields. UTC-labelled calendar filters retain the established application/database convention without fixed offsets or timestamp reinterpretation. Read monetary values in bounded chunks and use exact `Brick\Math\BigDecimal` string arithmetic, never SQL monetary SUM or PHP floats/native money casts. Stored-money validation permits zero; contribution validation requires positive SDG money.
- Invalid money/currency makes affected amounts unavailable. Missing/future/incoherent payment times make period results incomplete; never substitute another timestamp. Missing funding links also mark period breakdowns incomplete. Stored-total differences remain visible. Invalid delivery/completion data withholds affected delivered/balance figures; negative balances are never clamped. Display only generic aggregate integrity warnings, never corrupt values or private references. Empty valid results display zero counts and `0.00` SDG.
- Use scalar allowlisted queries and a presentation-only report DTO. Never select applicant identity, requested amount, documents, reviewer assignment, donor data, provider internals, idempotency/capability values, encrypted fields, messages, receiving details/methods, notes, or private UUID references. Proof checks use SQL existence predicates only. Report views never receive sensitive models or unrestricted rows.
- Record one minimal `reports.viewed` audit with the viewing actor only (no subject, filters, totals, period dates, request payload, references, or private information). Normal audit action/creation metadata remains. Audit failure rolls back the audit transaction and withholds the report using a generic private failure. The audit is the only business-database write; no lifecycle changes, notifications, or repairs occur.
- Apply the named 10-per-minute account throttle. All responses, including redirects, validation, authorization, throttle, and failures, use `Cache-Control: no-store, private`, `Pragma: no-cache`, `Referrer-Policy: no-referrer`, `X-Content-Type-Options: nosniff`, and `X-Robots-Tag: noindex, nofollow`. Errors must remain generic even with debug enabled and must not log request/SQL/private context.
- Add the `donations(status, paid_at, id)` index in a new migration; operational application requires separate authorization. Existing SQLite Campaign decimal affinity can lose stored precision at very large amounts; reports must flag unrepresentable stored values or ledger differences, not round donations to match.
- Retain the current bilingual accessible application layout with desktop/mobile authorized Reports links. Final design review may reuse the report DTO and definitions without changing financial semantics.

The automated test environment is isolated in-memory SQLite; no disposable MariaDB runtime test connection is configured. Before operational acceptance, verify report session-timezone preservation on a disposable MariaDB database with both UTC and non-UTC writer sessions, mixed TIMESTAMP/DATETIME fields, completed problem/resume histories, and donation midnight boundaries. Offline connection-contract tests do not substitute for that engine verification.
