<x-app-layout>
    ...
    <div class="py-12"><div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8"><div class="rounded-lg bg-white p-6 shadow-sm">
        @if (session('status') === 'campaign-created-from-help-application')
            <p role="status" class="mb-4 bg-green-50 p-4 text-sm text-green-800">Campaign draft created successfully. The Campaign remains unpublished. / <span lang="ar" dir="rtl">تم إنشاء مسودة الحملة بنجاح. تبقى الحملة غير منشورة.</span></p>
        @endif
        ...
        @if ($campaign->help_application_id === null && $currentCategory && ($currentCategory->trashed() || ! $currentCategory->is_active))<div role="alert" class="mb-4 rounded bg-amber-50 p-4 text-sm">The current Category is unavailable; choose an eligible Category before saving. / الفئة الحالية غير متاحة؛ اختر فئة متاحة قبل الحفظ.</div>@endif
        @if ($campaign->help_application_id === null && $categories->isEmpty())
        ...
        @else
            ...
            <form method="POST" action="{{ route('admin.campaigns.update', $campaign) }}" class="space-y-5">@csrf @method('PATCH')
                ...
                @if ($campaign->help_application_id !== null)<p>Inherited category / الفئة الموروثة: {{ $currentCategory?->name_en }} / {{ $currentCategory?->name_ar }}</p>@if ($currentCategory && (! $currentCategory->is_active || $currentCategory->trashed()))<p>Unavailable for new assignments / غير متاحة للتعيينات الجديدة</p>@endif @else <div><label for="category_id">Category / الفئة</label><select id="category_id" name="category_id" required class="mt-1 block w-full focus:ring-2 focus:ring-indigo-500" @if($errors->has('category_id')) aria-invalid="true" aria-describedby="category_id-error" @endif><option value="">Select / اختر</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected($oldCategory===(string)$category->id)>{{ $category->name_en }} / {{ $category->name_ar }}</option>@endforeach</select>@error('category_id')<p id="category_id-error">{{ $message }}</p>@enderror</div> @endif
