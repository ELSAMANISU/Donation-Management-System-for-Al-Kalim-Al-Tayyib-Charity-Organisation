@php
    ...
    $linkableErrorFields = [
        ...
        'public_identity_preference', 'document', 'consent',
    ];
...
<form method="POST" action="{{ $action }}" class="mt-6 space-y-8">
    ...
    <fieldset class="space-y-5">
        ...
        @foreach ([
            ...
            <div>
                ...
                <input id="{{ $field }}" name="{{ $field }}" type="{{ $meta[3] }}" value="{{ $value($field) }}" maxlength="{{ $meta[1] }}" autocomplete="{{ $meta[2] }}" aria-describedby="{{ $describedBy($field, $field.'-help') }}" @if($errors->has($field)) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has($field) ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
                @error($field)<p id="{{ $field }}-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
            </div>
        @endforeach
        ...
        <div>
            ...
            <textarea id="address" name="address" maxlength="2000" rows="3" autocomplete="street-address" aria-describedby="{{ $describedBy('address', 'address-help') }}" @if($errors->has('address')) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has('address') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">{{ $value('address') }}</textarea>
            @error('address')<p id="address-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
        </div>
        ...
        <div>
            ...
            <input id="date_of_birth" name="date_of_birth" type="date" value="{{ $value('date_of_birth') }}" autocomplete="bday" aria-describedby="{{ $describedBy('date_of_birth', 'date_of_birth-help') }}" @if($errors->has('date_of_birth')) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has('date_of_birth') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
            @error('date_of_birth')<p id="date_of_birth-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
        </div>
    </fieldset>
    ...
    <fieldset class="space-y-5">
        ...
        <div>
            ...
            <select id="identity_document_type" name="identity_document_type" aria-describedby="{{ $describedBy('identity_document_type', 'identity_document_type-help') }}" @if($errors->has('identity_document_type')) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has('identity_document_type') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
            ...
            @error('identity_document_type')<p id="identity_document_type-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
        </div>
        ...
        <div>
            ...
            <input id="identity_issuing_country" name="identity_issuing_country" value="{{ $value('identity_issuing_country') }}" maxlength="2" dir="ltr" aria-describedby="{{ $describedBy('identity_issuing_country', 'identity_issuing_country-help') }}" @if($errors->has('identity_issuing_country')) aria-invalid="true" @endif class="mt-1 block w-full rounded-md uppercase focus:ring-2 {{ $errors->has('identity_issuing_country') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
            @error('identity_issuing_country')<p id="identity_issuing_country-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
        </div>
        ...
        <div>
            ...
            <input id="identity_document_number" name="identity_document_number" value="" maxlength="255" autocomplete="off" dir="ltr" aria-describedby="{{ $describedBy('identity_document_number', 'identity_document_number-help') }}" @if($errors->has('identity_document_number')) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has('identity_document_number') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
            @error('identity_document_number')<p id="identity_document_number-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
        </div>
        ...
        @if ($application !== null)
            <div>
                ...
                @error('clear_identity_document_number')<p id="clear_identity_document_number-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
            </div>
        @endif
    </fieldset>
    ...
    <fieldset class="space-y-5">
        ...
        <div>
            ...
            <input id="requested_amount" name="requested_amount" value="{{ $value('requested_amount') }}" inputmode="decimal" dir="ltr" aria-describedby="{{ $describedBy('requested_amount', 'requested_amount-help') }}" @if($errors->has('requested_amount')) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has('requested_amount') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
            @error('requested_amount')<p id="requested_amount-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
        </div>
        ...
        @foreach ([
            ...
            <div>
                ...
                <textarea id="{{ $field }}" name="{{ $field }}" maxlength="{{ $meta[1] }}" rows="{{ $meta[2] }}" aria-describedby="{{ $describedBy($field, $field.'-help') }}" @if($errors->has($field)) aria-invalid="true" @endif class="mt-1 block w-full rounded-md focus:ring-2 {{ $errors->has($field) ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">{{ $value($field) }}</textarea>
                @error($field)<p id="{{ $field }}-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
            </div>
        @endforeach
    </fieldset>
    ...
    <fieldset class="space-y-4">
        ...
        <select id="public_identity_preference" name="public_identity_preference" aria-describedby="{{ $describedBy('public_identity_preference', 'public_identity_preference-help') }}" @if($errors->has('public_identity_preference')) aria-invalid="true" @endif class="block w-full rounded-md focus:ring-2 {{ $errors->has('public_identity_preference') ? 'border-red-200 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">
            <option value="">Not selected / غير محدد</option>
        ...
        @error('public_identity_preference')<p id="public_identity_preference-error" class="mt-1 text-sm text-red-800">{{ $message }}</p>@enderror
    </fieldset>
