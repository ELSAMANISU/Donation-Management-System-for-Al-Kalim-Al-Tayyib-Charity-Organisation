...
<head>
    ...
    <title>{{ $loc === 'ar' ? 'إنشاء حساب' : 'Create Account' }} — {{ config('app.name', 'Al-Kalimah Foundation') }}</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Form fields ── */
        .ls-field { margin-bottom: 1.05rem; }
        ...
        /* Password grid — two fields side-by-side on wider screens */
        .ls-pw-grid {
            display: grid;
        ...
        @media (max-width: 480px) {
            .ls-pw-grid { grid-template-columns: 1fr; }
        }
        ...
        .ls-signin-row a:hover { color: var(--amber); text-decoration: underline; }
        html[dir="rtl"] .ls-signin-row a { margin-left: 0; margin-right: 0.25rem; }
        ...
        @media (max-width: 520px) {
            ...
            .ls-card    { padding: 1.8rem 1.5rem; border-radius: 18px; }
...
{{-- ═══════════════════════════════════════════════
     MAIN — Glassmorphism Registration Card
═══════════════════════════════════════════════════ --}}
<main class="ls-main">

    <div class="ls-card"
         role="region"
         aria-label="{{ $loc === 'ar' ? 'إنشاء حساب جديد' : 'Create New Account' }}">

        {{-- Eyebrow + heading + rule --}}
        <span class="ls-eyebrow">
            @if($loc === 'ar') انضم إلينا @else New Member @endif
        </span>

        <h1 class="ls-heading">
            @if($loc === 'ar') إنشاء حساب @else Create an Account @endif
        </h1>

        <p class="ls-subheading">
            @if($loc === 'ar')
                انضم إلى رحلة العطاء والخير
            @else
                Join our journey of good &amp; giving
            @endif
        </p>
        ...
        <form method="POST" action="{{ route('register') }}" novalidate>
            ...
            {{-- Full Name --}}
            <div class="ls-field">
                <label class="ls-label" for="name">
                    @if($loc === 'ar') الاسم الكامل @else Full Name @endif
                </label>
                <input
                    class="ls-input"
                    id="name"
                    ...
                    name="name"
                    value="{{ old('name') }}"
                    placeholder="{{ $loc === 'ar' ? 'أدخل اسمك الكامل' : 'Enter your full name' }}"
                    required
                    autofocus
                    autocomplete="name"
                ...
                >
                @error('name')
                    <span class="ls-err" id="name-error" role="alert">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="ls-pw-grid">

                {{-- Password --}}
                <div class="ls-field" style="margin-bottom:0">
                ...
                {{-- Confirm Password --}}
                <div class="ls-field" style="margin-bottom:0">
                    <label class="ls-label" for="password_confirmation">
                        @if($loc === 'ar') تأكيد كلمة المرور @else Confirm Password @endif
                    </label>
                    <input
                        class="ls-input"
                        ...
                        required
                        autocomplete="new-password"
                        aria-describedby="{{ $errors->has('password_confirmation') ? 'pwc-error' : '' }}"
                    >
                    @error('password_confirmation')
                        <span class="ls-err" id="pwc-error" role="alert">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <button type="submit" class="ls-submit">
                ...
                @if($loc === 'ar') إنشاء الحساب @else Create Account @endif
            </button>

        </form>

        {{-- Sign in link --}}
        <p class="ls-signin-row">
            @if($loc === 'ar')
                هل لديك حساب بالفعل؟
